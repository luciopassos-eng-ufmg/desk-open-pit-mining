...
# ####################################################################################
# Projeto: Transporte em mina e britagens
# Autor: Lúcio Passos <lucio.passos@gmail.com>
# Descrição: 
# executa o programa

# desk-sim -m src\openpitmining.py --mode visualization
# desk-sim -m src\openpitmining.py --mode single
# desk-sim -m src\openpitmining.py --mode replications
# desk-sim -m src\openpitmining.py --mode factorial
# ##################################################################
...
def build_model(final_simulation_time=None, event_logger=None, verbose=False,
    ...
    def distribution(tipo):
        return {
            #----Frente de Lavra---------------
            # inicializa com todos caminhões disponíveis
            'chegadaCaminhoes': random.uniform(0, 0),
            # a chegada de ROM segue o ritmo de extração previsto nos dados que são
            # 8161827 toneladas produzidas, com cada caminhão podendo transportar em média 299,4 ton
            # em 31 dias, ou 44640 minutos
            # o que dá uma taxa de chegada de 44640/(8161827/299,4) = 1,64
            'chegadaROM': random.uniform(1.64, 1.64),
            # as pdfs abaixo foram obtidas a partir do registro histórico de ciclo de caminhões
            'carregamento_FL': (-8.43323 + 248.276 * random.betavariate(77.5073, 1506.59))*1.5,
            'mtbf_FL': 0.016666 + 496.441 * random.betavariate(0.503515, 4.53269),
            'mttr_FL': random.gammavariate(0.486516, 44.2687),
            #----Britagens----------
            # acrescentou-se 1 para considerar o tempo médio de espera para descarregar
            'delay_FL_BS1': 1 + (-0.270587) + 3.90734e+06 * random.betavariate(3.61331, 2.34131e+06),
            'delay_BS1_FL': -0.270587 + 3.90734e+06 * random.betavariate(3.61331, 2.34131e+06),
            # considerando 5913*4 ton/h como taxa nominal de 4 britadores, entidade=300 t
            # Taxa: 5913*4 / 300 = 78.84 entidades por hora
            # Convertendo para minutos: 78.84 / 80 = 0.76 min por entidade
            'recupera_brita_transp_BS1': random.uniform(0.76, 0.76),
...
#            'mttr_maquinario_7': (18.78 + random.weibullvariate(0.955, 7.879)),
            #----Caminhao----------
            #'mtbf_operador': random.gammavariate(0.683972, 52.0876),
            #'mttr_operador': random.expovariate(0.082185),   
            #autonomo
            'mtbf_operador': random.gammavariate(0.78299, 79.411),
            'mttr_operador': random.weibullvariate(9.93131, 0.942199),     
        }.get(tipo,0.0)
...
def run_replications():
    ...
    if CUSTOM_REPLICATION_KPIS:
        ...
        df_relatorio.to_csv(
            "results/relatorio_replicacoes_sbpo.csv",
            index=False,
            sep=";",
            decimal=","
        )
